...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user) {
      ...
      const responsePayload = buildResponsePayload(listing, analysis)
    ...
    if (cached?.raw_listing_data) {
      ...
      if (age < 86400000) {
        return NextResponse.json({
          listing: cached.raw_listing_data.listing,
          analysis: cached.raw_listing_data.analysis,
          cached: true,
    ...
    const { data: savedAnalysis } = await supabase.from('analyses').insert({
      user_id: user.id,
      flippa_url: url,
      ...
      score_breakdown: analysis.flip_score,
      red_flags: analysis.red_flags,
      growth_opportunities: analysis.growth_opportunities,
      recommendation: analysis.valuation.recommendation,
      summary: analysis.summary,
      raw_listing_data: responsePayload,
    }).select().single()

    await supabase.from('profiles')
      .update({ analyses_used_this_month: used + 1 }).eq('id', user.id)

    return NextResponse.json({
      ...responsePayload,
      analysis: {
        ...responsePayload.analysis,
        flip_plan: hasAdvanced ? responsePayload.analysis.flip_plan : null,
        roi_scenarios: hasAdvanced ? responsePayload.analysis.roi_scenarios : null,
        negotiation: hasAdvanced ? responsePayload.analysis.negotiation : null,
      },
...
function buildResponsePayload(listing: any, analysis: any) {
  return {
    listing: {
      title: listing.title,
      url: listing.url,
...
async function markGuestUsed(token: string, supabase: any): Promise<void> {
  try {
    await supabase.from('guest_analyses').insert({ token })
  } catch { /* best effort */ }
}
